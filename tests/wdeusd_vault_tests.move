...
use elixir::test_wdeusd::{Self, TEST_WDEUSD};
...
fun test_initialize_vault() {
    let (mut scenario, deusd_config, admin_cap, coin_metadata) = setup_test(6);

    scenario.next_tx(ADMIN);
    {
        wdeusd_vault::initialize<TEST_WDEUSD>(&admin_cap, &coin_metadata, scenario.ctx());
    };

    scenario.next_tx(ADMIN);
    {
        // Verify vault was created and shared
        let vault = ts::take_shared<WDEUSDVault<TEST_WDEUSD>>(&scenario);
    ...
    ts::return_shared(coin_metadata);
    scenario.end();
}


#[test]
#[expected_failure(abort_code = wdeusd_vault::EDeUSDDecimalsMismatch)]
fun test_initialize_vault_fail_if_wdeusd_mismatch_deusd_decimals() {
    // deUSD has 6 decimals, so using 8 here to trigger mismatch
    let (mut scenario, deusd_config, admin_cap, coin_metadata) = setup_test(8);
...
fun test_pause_vault_success() {
    let (mut scenario, deusd_config, admin_cap, wdeusd_metadata) = setup_test(6);

    scenario.next_tx(ADMIN);
    {
        wdeusd_vault::initialize<TEST_WDEUSD>(&admin_cap, &wdeusd_metadata, scenario.ctx());
    };

    scenario.next_tx(ADMIN);
    {
        let mut vault = ts::take_shared<WDEUSDVault<TEST_WDEUSD>>(&scenario);
    ...
    ts::return_shared(deusd_config);
    sui::test_utils::destroy(admin_cap);
    ts::return_shared(wdeusd_metadata);
    scenario.end();
}
...
fun test_claim_deusd_success() {
    let (mut scenario, mut deusd_config, admin_cap, wdeusd_metadata) = setup_test(6);
...
fun test_return_deusd_success() {
    ...
    scenario.next_tx(USER2);
    {
        ...
        let wdeusd_coin = ts::take_from_address<Coin<TEST_WDEUSD>>(&scenario, USER2);
...
fun setup_test(wdeusd_decimals: u8): (Scenario, DeUSDConfig, AdminCap, CoinMetadata<TEST_WDEUSD>) {
    ...
    // Create currency properly using init pattern
    scenario.next_tx(ADMIN);
    test_wdeusd::init_test_wdeusd(wdeusd_decimals, scenario.ctx());

    scenario.next_tx(ADMIN);
    let wdeusd_metadata = ts::take_shared<CoinMetadata<TEST_WDEUSD>>(&scenario);

    (scenario, deusd_config, admin_cap, wdeusd_metadata)
}

fun mint_test_wdeusd(amount: u64, ctx: &mut TxContext): Coin<TEST_WDEUSD> {
    coin::mint_for_testing<TEST_WDEUSD>(amount, ctx)
}
